...
export const PATCH = async(request: Request)=> {
    try {
        ...
        if(!userId || !newUserName){
            return new NextResponse(JSON.stringify({message: "ID or new username is required!"}),
            {status: 400}
            );
        }
        ...
        return new NextResponse(JSON.stringify({message: "user is updated successfully", user: updatedUser}), {
            status: 200
        })
    } catch (error: any) {
